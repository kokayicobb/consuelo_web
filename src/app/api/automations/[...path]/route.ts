...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  if (path.length === 3 && path[0] === 'flows' && path[2] === 'runs') {
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, error: { message: error.message || 'Failed to get flow runs' } },
        { status: 500 }
      );
    }
  }
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  if (path.length === 1 && path[0] === 'flows') {
    ...
    try {
      ...
      console.log('Creating flow with data:', JSON.stringify(createData, null, 2));
  ...
  if (path.length === 2 && path[0] === 'flows') {
    ...
    try {
      ...
      console.log('Updating flow with data:', JSON.stringify(updateData, null, 2));
  ...
  if (path.length === 3 && path[0] === 'flows' && path[2] === 'test') {
    ...
    try {
      ...
      const result = await activePieces.testWebhookTrigger(flowId, body.data || {});
      return NextResponse.json(result);
    } catch (error: any) {
      console.error(`❌ Error testing flow ${flowId}:`, error);
      return NextResponse.json(
        { success: false, error: { message: error.message || 'Failed to test flow' } },
        { status: 500 }
      );
    }
  }
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  const { path } = params;
  console.log(`✅ Catch-all route running: PUT /api/automations/${path.join('/')}`);
