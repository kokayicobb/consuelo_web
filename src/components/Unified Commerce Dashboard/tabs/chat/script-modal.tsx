...
export default function ScriptModal({
  ...
  const client = {
    ...
    lastVisit:
      clientData?.lastVisit || clientData?.clientInfo?.["Last Visit"] || "",
    membershipType:
  ...
  const firstName =
    client.name?.split(", ")[1] || client.name || clientName || "";
  ...
  const findTextToHighlight = useCallback(
    (
      ...
      // For closing lines
      if (
        suggestion.suggested.includes("thank") ||
  ...
  const fetchTalkingPoints = useCallback(async (scriptToAnalyze: string) => {
    ...
    try {
      console.log(
        "Requesting talking points for script:",
        scriptToAnalyze.substring(0, 50) + "...",
      );
  ...
  const generateScriptSuggestions = useCallback(
    async (scriptToAnalyze: string) => {
  ...
  const handleGenerateScript = useCallback(
    async (newScriptType: "call" | "email") => {
      ...
      try {
        ...
        console.log(
          "Generated script received:",
          generatedScript.substring(0, 50) + "...",
        );
  ...
  const applySuggestion = useCallback(
    (suggestion: ScriptSuggestion) => {
      ...
      setEditableScript((current) => {
        ...
        if (
          !suggestion.original &&
          (suggestion.type === "tone" || suggestion.type === "clarity")
        ) {
  ...
  useEffect(() => {
    ...
    if (
      (Math.abs(editableScript.length - lastAnalyzedScript.length) < 10 &&
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (initialScript) {
        ...
        console.log(
          "No initial script, generating new one for initial tab:",
          scriptType,
        );
    ...
    if (hoveredSuggestion) {
      ...
      if (textarea) {
        const highlightContainer =
          textarea.parentElement?.querySelector(".absolute.inset-0");
  ...
  return (
    <Dialog as={Fragment} open={isOpen} onClose={onClose}>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
        <Dialog.Panel className="relative flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-lg bg-white p-0 shadow-xl">
          {/* Header */}
          <div className="sticky top-0 z-10 flex items-center justify-between border-b border-sky-100 bg-sky-50 p-4">
            <Dialog.Title className="text-xl font-semibold text-gray-900">
              Client Communication Script
            </Dialog.Title>
            <button
              onClick={onClose}
              className="rounded text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-sky-500"
            >
              <X size={24} />
            </button>
          </div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              ...
              <div className="lg:col-span-1">
                <div className="flex h-full flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                  <div className="border-b border-gray-200 bg-gradient-to-r from-sky-50 to-sky-100 p-4">
                  ...
                  <div className="flex flex-grow flex-col space-y-5 p-4">
                    {/* Contact Information */}
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {client.phone && (
                          <div className="group flex items-center">
                            <Phone className="mr-2 h-4 w-4 text-gray-400 transition-colors group-hover:text-sky-600" />
                            <a
                              href={`tel:${client.phone}`}
                              className="text-sm text-gray-700 hover:text-sky-600 hover:underline"
                            >
                              {client.phone}
                            </a>
                          </div>
                        ...
                        {client.email && (
                          <div className="group flex items-center">
                            <Mail className="mr-2 h-4 w-4 text-gray-400 transition-colors group-hover:text-sky-600" />
                            <a
                              href={`mailto:${client.email}`}
                              className="truncate text-sm text-gray-700 hover:text-sky-600 hover:underline"
                              title={client.email}
                            >
                              {client.email}
                            </a>
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        ...
                        {client.expirationDate && (
                          <div className="flex items-center">
                            ...
                            {isExpiringSoon && (
                              <span className="ml-2 rounded-full border border-amber-200 bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800">
                                Soon
                              </span>
                            )}
                          </div>
                    ...
                    {contactLogs && contactLogs.length > 0 && (
                      <div className="space-y-2">
                        ...
                        >
                          <h4 className="flex items-center gap-1 text-xs font-semibold uppercase tracking-wider text-gray-500 transition-colors group-hover:text-sky-600">
                        ...
                        {expandedLogs && (
                          <div className="mt-2 max-h-40 space-y-3 overflow-y-auto border-l-2 border-gray-100 pl-3 pr-2">
                    ...
                    <div className="mt-4 flex h-full flex-grow flex-col space-y-2">
                      <div
                        className="group flex cursor-pointer items-center justify-between"
                        onClick={() =>
                      ...
                      >
                        <h4 className="flex items-center gap-1 text-xs font-semibold uppercase tracking-wider text-gray-500 transition-colors group-hover:text-sky-600">
                          ...
                          AI Script Suggestions
                          {isGeneratingSuggestions && (
                            <Loader2 className="ml-1 h-3 w-3 animate-spin text-sky-500" />
                          )}
                        </h4>
                        <ChevronRight
                          className={`h-4 w-4 text-gray-400 transition-transform group-hover:text-sky-600 ${
                      ...
                      {expandedSuggestions && (
                        <div
                          className="mt-2 flex-grow space-y-3 overflow-y-auto border-l-2 border-sky-100 pl-3 pr-2"
                          style={{ minHeight: "200px" }}
                        >
                          {isGeneratingSuggestions ? (
                            <div className="space-y-2 py-2">
                              <div className="h-3 w-3/4 animate-pulse rounded bg-sky-50"></div>
                              <div className="h-3 w-5/6 animate-pulse rounded bg-sky-50"></div>
                              <div className="h-3 w-1/2 animate-pulse rounded bg-sky-50"></div>
                            </div>
                          ) : scriptSuggestions.length > 0 ? (
                            scriptSuggestions.map((suggestion, index) => (
                              <div
                                key={index}
                                className="group relative cursor-pointer rounded px-2 py-2 transition-colors hover:bg-sky-50"
                              ...
                              >
                                <div className="mb-0.5 flex items-start justify-between">
                                  <div className="flex items-center">
                                    <Edit className="mr-1.5 h-3.5 w-3.5 flex-shrink-0 text-sky-600" />
                                    <span
                                  ...
                                  <span className="rounded bg-sky-50 px-1.5 py-0.5 text-xs text-sky-600 opacity-0 transition-opacity group-hover:opacity-100">
                                    Apply
                                  </span>
                                </div>
                                <p className="ml-1 mt-1.5 border-l-2 border-sky-200 pl-2 text-xs leading-relaxed text-gray-700">
                                  "{suggestion.suggested}"
                                </p>
                                {suggestion.reason && (
                                ...
                                {index < scriptSuggestions.length - 1 && (
                                  <hr className="mt-2 border-sky-100" />
                                )}
                              </div>
                            ))
                          ) : (
              ...
              <div className="lg:col-span-2">
                <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                  ...
                  <div className="p-4">
                    ...
                    <div className="mb-4 border-b border-gray-200">
                      <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                        <button
                          ...
                          className={`flex items-center whitespace-nowrap border-b-2 px-1 pb-3 text-sm font-medium transition-colors duration-150 ease-in-out ${
                            activeTab === "call"
                              ? "border-sky-500 text-sky-600"
                              : "border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700"
                          }`}
                          aria-current={
                            activeTab === "call" ? "page" : undefined
                          }
                        >
                        ...
                        <button
                          ...
                          aria-current={
                            activeTab === "email" ? "page" : undefined
                          }
                        >
                    ...
                    <div className="mb-5 rounded-lg border border-sky-100 bg-gradient-to-r from-sky-50 to-sky-100 p-4 shadow-sm">
                      <h4 className="mb-3 flex items-center text-sm font-semibold text-sky-800">
                        <svg
                        ...
                        Key Talking Points
                        {isGeneratingPoints && (
                          <Loader2 className="ml-2 h-4 w-4 animate-spin text-sky-500" />
                        )}
                      </h4>
                      {isGeneratingPoints ? (
                        <div className="space-y-2">
                          <div className="h-3 w-3/4 animate-pulse rounded bg-sky-100"></div>
                          <div className="h-3 w-5/6 animate-pulse rounded bg-sky-100"></div>
                          <div className="h-3 w-1/2 animate-pulse rounded bg-sky-100"></div>
                        </div>
                      ) : pointsError ? (
                        <div className="flex items-center rounded border border-red-200 bg-red-100 p-3 text-sm text-red-700">
                        ...
                        <ul className="space-y-2">
                          {talkingPoints.map((point, pointIdx) => (
                            <li key={pointIdx} className="flex items-start">
                              <Check className="mr-2 mt-0.5 h-4 w-4 flex-shrink-0 text-sky-500" />
                              <span className="text-sm text-gray-700">
                                {point}
                              </span>
                            </li>
                        ...
                        <p className="text-sm italic text-gray-500">
                          No key talking points generated for this script.
                        </p>
                      ) : null}
                    </div>
                    ...
                    <div className="relative rounded-md border border-gray-200 bg-white shadow-inner">
                        ...
                        {isGeneratingScript ? (
                          ...
                          <div className="relative min-h-[200px]">
                            <button
                              ...
                              className="absolute right-0 top-0 z-10 mr-1 mt-1 rounded-full p-1.5 text-gray-400 transition-colors duration-150 ease-in-out hover:bg-sky-100 hover:text-sky-600 focus:outline-none focus:ring-2 focus:ring-sky-300"
                              title="Copy script"
                            >
                              {isCopied ? (
                            ...
                            <textarea
                              ...
                              className={`w-full resize-none border-none bg-transparent pr-8 pt-8 text-sm leading-relaxed text-gray-800 focus:outline-none focus:ring-0 ${
                                hoveredSuggestion
                              ...
                              placeholder={
                                isGeneratingScript
                                  ? "Generating script..."
                                  : "No script generated. Type here to create one."
                              }
                              spellCheck={true}
                              style={{ minHeight: "200px" }}
                            />
                            ...
                            {hoveredSuggestion && (
                              <div
                                className="absolute inset-0 overflow-auto pr-8 pt-8 text-sm leading-relaxed text-gray-800"
                                style={{ minHeight: "200px" }}
                              >
                                {(() => {
                                  const highlightInfo = findTextToHighlight(
                                    hoveredSuggestion,
                                    editableScript,
                                  );

                                  if (highlightInfo) {
                                    const beforeText = editableScript.substring(
                                      0,
                                      highlightInfo.start,
                                    );
                                    const highlightedText =
                                      editableScript.substring(
                                        highlightInfo.start,
                                        highlightInfo.end,
                                      );
                                    const afterText = editableScript.substring(
                                      highlightInfo.end,
                                    );
                                    ...
                                    return (
                                      <>
                                        ...
                                        <span className="whitespace-pre-wrap border-b-2 border-sky-400 bg-sky-100">
                                          {highlightedText}
                                        </span>
                                        <span className="whitespace-pre-wrap">
                                          {afterText}
                    ...
                    {!isGeneratingScript && (
                      <div className="mt-3 flex justify-end">
                        <button
                          ...
                          className="flex items-center rounded-md bg-sky-100 px-3 py-1.5 text-sm font-medium text-sky-700 transition-colors duration-150 hover:bg-sky-200"
                        >
                          {isSaved ? (
                            <>
