...
function AIResponsePanel({ message }: { message: Message }) {
  return (
    <ScrollArea className="h-full">
      ...
      <div className="prose prose-sm max-w-none prose-headings:text-foreground prose-p:text-foreground prose-strong:text-foreground prose-li:text-foreground prose-blockquote:text-muted-foreground prose-code:text-foreground prose-pre:text-foreground">
  <MarkdownContent 
    id={`panel-${message.id}`} 
...
export function ConversationalInterface({
  ...
  useEffect(() => {
    // Only scroll to the start of the newest message, not the very bottom
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      // Get AI response with artifacts
      const { content, artifacts } =
        await sendChatMessageInsights(conversationHistory);
  ...
  return (
    <div className="flex h-screen">
      ...
      >
        ...
        {/* Messages */}
        <ChatMessageArea className="space-y-4 px-4 py-8">
          <div className="space-y-4">
            {messages.map((message, index) => (
              ...
              >
                ...
                >
                  <Card
                    className={`${message.role === "user" ? "bg-blue-500 text-white" : "bg-muted"}`}
                  >
  ...
  {message.hasArtifacts && (
    <div className="mt-2 pt-2 border-t border-border/50">
      <Button
        ...
        className="text-xs"
      >
        View Interactive Response ({message.artifacts?.length} item{message.artifacts?.length !== 1 ? 's' : ''})
      </Button>
    </div>
  )}
</CardContent>
